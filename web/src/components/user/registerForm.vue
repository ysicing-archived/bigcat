...
<template>
      <a-form ref="formRef" :model="registerForm" :rules="rules" v-bind="layout">
            ...
            <a-form-item :label="$t('user.form.confirm')" name="confirm_password" has-feedback>
            ...
            <a-form-item :label="$t('common.action')">
                  <a-button type="primary" @click="registered">{{ $t('user.form.title') }}</a-button>
            </a-form-item>
      </a-form>
</template>
...
const rules = {
      username: [{ trigger: 'blur', message: t('user.form.valid.user'), required: true }],
